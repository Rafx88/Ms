...
type SupportedQueryEngine = 'google' | 'wikipedia' | 'reddit' | 'local' | 'gemini'
...
export class QueryCore {
    ...
    async queryManager(
        ...
        const {
            shuffle = false,
            sourceOrder = ['gemini', 'google', 'wikipedia', 'reddit', 'local'] as QueryEngine[],
            related = true,
            langCode = 'en',
            geoLocale = 'US'
        } = options
        ...
        try {
            ...
            for (const source of sourceOrder) {
                const handler = sourceHandlers[source as SupportedQueryEngine]
                if (!handler) continue
    ...
    async getGeminiQueries(langCode = 'en', geoLocale = 'US'): Promise<string[]> {
        const apiKey = 'AIzaSyCkoxKwDAlDB7vjnze5f5D_qCFOf5J_h2U'

        try {
            const prompt = `
Generate 80 short, natural web search queries.
...
- Region: ${geoLocale}
- Mix topics: news, tech, sports, entertainment, finance, health, education, food, travel, daily life.
- Avoid adult, illegal, violent, hateful, or sensitive topics.
- Avoid duplicate or very similar queries.
- Queries should look like normal human search phrases.
Example output:
["latest technology news", "best food near me", "how to save battery on android"]
`.trim()
            ...
            if (!text) {
                this.bot.logger.debug(this.bot.isMobile, 'SEARCH-GEMINI', 'empty text response')
                return []
            }
            ...
            try {
                ...
                if (!match) {
                    this.bot.logger.debug(
                        this.bot.isMobile,
                        'SEARCH-GEMINI',
                        `invalid JSON response: ${cleaned.slice(0, 300)}`
                    )
                    return []
                }
            ...
            if (!out.length) {
                this.bot.logger.debug(this.bot.isMobile, 'SEARCH-GEMINI', 'parsed JSON but result empty')
                return []
            }

            const final = this.normalizeAndDedupe(out)

            this.bot.logger.debug(
                this.bot.isMobile,
                'SEARCH-GEMINI',
                `gemini queries loaded | raw=${out.length} | final=${final.length}`
            )
